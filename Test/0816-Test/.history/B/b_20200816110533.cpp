...
signed main()
{
    ...
    while (q--)
    {
        ...
        {
            if (tmp2>=tmp1)
                print(query(tmp1+1,tmp2-1,1,n,1)*(1<<(tmp2-tmp1))+(/*query(tmp1,tmp1,1,n,1)+(tmp1!=tmp2)*query(tmp2,tmp2,1,n,1)*/(tmp1==tmp2)?query(tmp1,tmp1,1,n,1):query(tmp1,tmp1,1,n,1)+query(tmp2,tmp2,1,n,1))*(tmp2-tmp1+1),'\n');
            else
                print(0,'\n');
        }        
    }
